...
/* ######################################################## */
PRIVATE int alloc_type = FIRST_FIT;
/* ######################################################## */
 ...
 */
  register struct hole *hp, *prev_ptr, *max_ptr, *prev_max_ptr;
  phys_clicks old_base;

  do {
    prev_ptr = prev_max_ptr = NIL_HOLE;
    hp = max_ptr = hole_head;

    while (hp != NIL_HOLE && hp->h_base < swap_base) {
      if (alloc_type == FIRST_FIT && hp->h_len >= clicks) {
